...
class SpectrumCalculator:
    # this doesn't really need to be a class, but for now it's a
    # useful way of keeping a lot of related functions together
    ...
    def calc_spectrum(self, gs_dict, es_dict, **kwargs):
        ...
        if spec_type == 'Excitation':
            ...
            for m, zval in enumerate(gs_dict['eigenvalues']):
                for n, yval in enumerate(es_dict['eigenvalues']):
                    ...
                    temp_ind = self.find_ind(z1y1_freq + yval - zval, freq_axis)
    ...
    @staticmethod
    def convolve_spectrum(freq_axis, spec_in, linewidth):
        # convolves the spectrum with a single lorentzian
        # to do in future updates - different lineshapes, state-dependent linewidth

        x0 = np.mean(freq_axis)
        lineshape = 1 / ((freq_axis - x0) ** 2 + 0.5 * linewidth ** 2)
        lineshape = lineshape / np.sum(lineshape)
        spec_conv = np.convolve(spec_in, lineshape,'same')
